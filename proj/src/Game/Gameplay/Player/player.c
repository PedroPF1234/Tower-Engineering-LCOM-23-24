...
void updatePlayerPosition(Player* player, Arena arena) {
  ...
  // Collision Detection 
  int16_t shop_left_corner = arena.shop.shopObject->x - 75;
  int16_t shop_right_corner = arena.shop.shopObject->y + 75;

  if (new_x > shop_left_corner && new_x < shop_right_corner && new_y <= arena.shop.shopObject->y && new_y >= arena.shop.shopObject->y - 100) {
    can_move = false;
  }
